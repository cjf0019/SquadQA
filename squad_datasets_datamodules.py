...
class NLPDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        example = self.retrieve_text(
            idx)  # returns dict of text_col names to the text... concatenation of multiple cols also returned

        ### TOKENIZE
        if self.run_tokenizer_on_output:
            for text in example.keys():
                example[text] = tokenize_to_dict(self.tokenizer, example[text], self.sentence_len,
    ...
    def index_text_samples(self, text_cols):
        ...
        if self.aggregate_by == 'column':
            ...
            for col in text_cols:
                strcol = '+'.join(
                    col)  # for cases where a tuple or list of columns is supplied, such that their text will be grouped together
                ...
                self.df[strcol + '_IDX_Start'] = self.df[strcol + '_NumSentences'].cum_sum() + idx_start
                # else:
    ...
    def calculate_num_sentences_textcol(df, tokenizer, text_col):
        ...
        df[strcol + '_NumSentences'] = NLPDataset.concat_text_cols(df, text_col).transform(lambda x:
                                                                                           NLPDataset.calculate_num_sentences(
                                                                                               x, tokenizer))
        return df
